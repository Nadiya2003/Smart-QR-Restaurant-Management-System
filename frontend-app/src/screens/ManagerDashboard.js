...
const ManagerDashboard = () => {
    ...
    }, [activeTab, token, attendanceDate, filterResDate, filterTableDate, filterTableTime]);
    useEffect(() => {
        fetchData();
    }, [activeTab, fetchData]);
...
const renderOrders = () => (
    ...
    const renderReports = () => {
        ...
        const handleDownloadPDF = () => {
            ...
            const pdfUrl = `${apiConfig.API_URL}/reports/pdf?${params}`;
                    ...
                    <View style={{ flexDirection: 'row', gap: 10, marginVertical: 12 }}>
                        {['today', 'weekly', 'monthly'].map(r => (
                            <TouchableOpacity key={r} onPress={() => setQuickRange(r)}
                                ...
                                <Text style={{ fontSize: 11, fontWeight: 'bold', color: '#374151', textTransform: 'uppercase' }}>{r}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
            ...
            <TouchableOpacity 
                ...
                onPress={() => {
                    Alert.alert(
                        ...
                        [
                            { text: 'Cancel', style: 'cancel' },
                            { text: 'ARRIVED', onPress: () => updateResStatus(res.id, 'ARRIVED') },
                            { text: 'COMPLETED', onPress: () => updateResStatus(res.id, 'COMPLETED') },
                            { text: 'NO SHOW', onPress: () => updateResStatus(res.id, 'NOSHOW') },
                            { text: 'CANCEL BOOKING', style: 'destructive', onPress: () => updateResStatus(res.id, 'CANCELLED') }
                        ]
                    );
                }}
            >
    ...
    const renderTables = () => {
        ...
        return (
            <>
                ...
                {/* Date/Time Filter Row for Tables */}
    ...
    const handleItemRemovalAction = async (id, action) => {
        try {
            const res = await fetch(`${apiConfig.API_BASE_URL}/api/orders/removal-requests/${id}/${action}`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ notes: 'Processed via Mobile Manager' })
            });
            if (res.ok) {
                ...
                <View style={styles.adminInfo_compact}>
                    <TouchableOpacity onPress={() => setActiveTab('account')} style={[styles.adminAvatar_small, { overflow: 'hidden' }]}>
                        {user?.profile_image ? (
                            ...
                            <Text style={styles.avatarLetter_small}>{user?.name?.charAt(0).toUpperCase() || 'M'}</Text>
                        )}
                    </TouchableOpacity>
