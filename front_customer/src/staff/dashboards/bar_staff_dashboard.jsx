...
const BarStaffDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (!token || userData.role !== 'bar_staff') {
    ...
    const fetchDashboardData = async (token) => {
        try {
            ...
            const [notifRes, ordersRes] = await Promise.all([
                ...
                fetch('http://192.168.1.3:5000/api/staff/orders?category=drinks', { headers }),
            ]);
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="flex justify-between items-center mb-8">
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-full bg-gradient-to-r from-pink-500 to-pink-600 flex items-center justify-center text-2xl">
                            {user?.profile_image ? (
                        ...
                        <div>
                            ...
                            <p className="text-white/60">Bar Staff Dashboard</p>
                        </div>
                    </div>
                    ...
                    <GlassCard>
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-pink-400">{stats.preparingOrders}</p>
                            </div>
                        </div>
                    </GlassCard>
                                        ...
                                        <div key={order.id} className="bg-white/5 rounded-xl p-4 border border-white/10">
                                            <div className="flex justify-between items-start mb-3">
                                                ...
                                                <span className={`px-3 py-1 rounded-lg text-xs font-semibold ${order.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400' :
                                                        order.status === 'preparing' ? 'bg-pink-500/20 text-pink-400' :
...
export default BarStaffDashboard;
