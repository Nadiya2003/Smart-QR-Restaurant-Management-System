...
function TableSelection({ areaId, date, time, selectedTableId, onSelectTable }) {
    ...
    const fetchAvailability = async (isSilent = false) => {
        if (!isSilent) setLoading(true);
        setError('');
        try {
            const url = areaId 
                ? `${config.API_BASE_URL}/api/reservations/availability?area_id=${areaId}&date=${date}&time=${time}`
                : `${config.API_BASE_URL}/api/reservations/availability?date=${date}&time=${time}`;
        ...
        <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            ...
            <div className="flex flex-wrap justify-center gap-4 text-xs mt-8 pb-4">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                    ...
                    <span className="text-gray-300 font-medium">Reserved / Active</span>
                </div>
            </div>
